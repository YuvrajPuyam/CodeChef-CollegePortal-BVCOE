...
def ranking(request):
    template = 'main/collegeleaderboard.html'
    # paginate_by = 50
    page_num = request.GET.get('page',1)
    p = Paginator(rank.objects.all().order_by('collegerank'),25)
...
def user_profile(request, username):
    user = rank.objects.filter(username=username).first()
    if user == None:
        template = 'core/usernotfound.html'
        context = {}
    else :
        ...
        'rating': user.rating,
        'countryrank' : user.countryranking,
        'globalrank' : user.globalranking,
        'stars': user.stars,
        }
        
    return render(request,template,context)

def contestleaderboard(request, contestid):
    contest = pastcontest.objects.filter(code=contestid.upper()).first()
    if contest == None:
        template = 'core/contestnotfound.html'
        context = {}
    else : 
        contestranks = pastrank.objects.filter(pastcontest=contest)
        context = {
            'contest' : contest,
            'ranks' : contestranks             
        }
        template = 'main/contestleaderboard.html'
    return render(request, template,context)
...
def blog(request):
    template = 'main/blog_landing.html'
    return render(request,template)

def blogpage(request):
    blog_num = request.GET.get('page',1)
    template = f'main/blog{blog_num}.html'
    return render(request,template)

def toupdate(request):
    return render(request,'main/to_be_updated.html')